...
def _bulk_intern(values, times, omegas, ft_sign, time_zero, weights=None, lin_weights=False):
    ...
    if weights is None:
        for i in prange(len(values)):
            results[i, :len(omegas[i])] = ft_uneven(values[i], times[i], omegas[i], ft_sign, time_zero, weights=None,
                                                    lin_weights=lin_weights)
    else:
        for i in prange(len(values)):
            results[i, :len(omegas[i])] = ft_uneven(values[i], times[i], omegas[i], ft_sign, time_zero,
                                                    weights=weights[i], lin_weights=lin_weights)
    return results
...
def _bulk_intern_fixed_omg(values, times, omegas, ft_sign, time_zero, weights=None, lin_weights=False):
    ...
    if weights is None:
        for i in range(len(values)):
            results[i] = ft_uneven(values[i], times[i], omegas, ft_sign, time_zero, weights=None,
                                   lin_weights=lin_weights)
    else:
        for i in range(len(values)):
            results[i] = ft_uneven(values[i], times[i], omegas, ft_sign, time_zero, weights=weights[i],
                                   lin_weights=lin_weights)
    return results
...
def _bulk_intern_fixed_time():
    ...
    if weights is None:
        for i in prange(len(values)):
            results[i, :len(omegas[i])] = ft_uneven(values[i], times, omegas[i], ft_sign, time_zero, weights=None,
                                                    lin_weights=lin_weights)
    else:
        for i in prange(len(values)):
            results[i, :len(omegas[i])] = ft_uneven(values[i], times, omegas[i], ft_sign, time_zero, weights=weights[i],
                                                    lin_weights=lin_weights)
    return results
...
def _bulk_intern_fixed_time_omg(values, times, omegas, ft_sign, time_zero, weights=None, lin_weights=False):
    ...
    if weights is None:
        ...
        for i in prange(len(values)):
            results[i] = ft_uneven(values[i], times, omegas, ft_sign, time_zero, weights=weights[i],
                                   lin_weights=lin_weights)
    return results
...
def ft_uneven_bulk_adaptive(values, times, omegas, ft_sign, time_zero, weights=None, lin_weights=False,
                            multithreading=True):
    ...
    results = func_dict[(omg_form, t_form, multithreading)](values, times, omegas, ft_sign, time_zero, weights=weights,
                                                            lin_weights=False)
